...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        #logn_list = [
        #    torch.log(i, self.seq_length) if i > self.seq_length else 1
        #    for i in range(1, 32768)
        #]
        logn_tensor = torch.arange(0, 32768, dtype=torch.float32)
        logn_tensor = torch.where(logn_tensor>self.seq_length, torch.log2(logn_tensor) / 13, 1)
    ...
    def forward(
        ...
        if rotary_pos_emb_list is not None:
            ...
            if len(rotary_pos_emb_list) == 1:
                ...
                rotary_pos_emb = [i[:, -1*cur_len:, :, :] for i in rotary_pos_emb]
        ...
        if layer_past is not None:
            ...
            if self.use_cache_quantization:
                ...
                # present=(key,value)
                #past_key = torch.transpose(past_key,1,2)
                #past_value = torch.transpose(past_value,1,2)
        ...
        if (
            ...
            if query.size(1) == key_size:
                ...
                ).unsqueeze(0).unsqueeze(0)#view(1, 1, key_size, key_size)
            else:
                causal_mask = None
...
class QWenModel(QWenPreTrainedModel):
    ...
    def get_ntk_alpha(self, true_seq_len):
        ...
        context_value = torch.tensor(true_seq_len/self.seq_length)
        context_value = torch.log2(context_value) + 1
        context_value = torch.ceil(context_value)
