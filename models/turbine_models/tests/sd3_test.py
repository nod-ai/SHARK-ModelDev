...
def command_line_args(request):
    ...
    arguments["hf_model_name"] = "stabilityai/stable-diffusion-3-medium-diffusers"
    ...
    arguments["device"] = utils.iree_device_map(request.config.getoption("--device"))
    arguments["backend"] = utils.iree_backend_map(
        request.config.getoption("--device").split("://")[0]
    )
    ...
    # TODO (Ean Garvey): align attention spec handling so we don't have to do this.
    if not arguments["attn_spec"] and not arguments["decomp_attn"]:
        if "gfx9" in arguments["iree_target_triple"]:
            arguments["attn_spec"] = "mfma"
        elif "gfx11" in arguments["iree_target_triple"]:
            arguments["attn_spec"] = "wmma"
...
class StableDiffusion3Test(unittest.TestCase):
    ...
    @pytest.mark.xfail(reason="Numerics issues on ~.01 percent of output values")
    def test01_ExportPromptEncoder(self):
        if arguments["device"] in ["vulkan", "cuda"]:
            self.skipTest("Not testing sd3 on vk or cuda")
        arguments["external_weight_path"] = (
            self.safe_model_name + "_text_encoders_" + arguments["precision"] + ".irpa"
        )
        prompt_encoder_vmfb = sd3_text_encoders.export_text_encoders(
    ...
    @pytest.mark.xfail(
        reason="Runners need secure dedicated access to gated HF repo for imports."
    )
    ...
    @pytest.mark.skip(
        reason="Waiting on inference plumbing for generalized sd pipeline"
    )
