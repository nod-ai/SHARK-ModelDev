...
class CompiledContext(BaseContext):
    ...
    # TODO: These handlers are by now all very similar and could be autogenerated
    def handle_alloc_shared(self, op, shape, dtype):
        node = AllocSharedNode(self.region_graph, op, shape, dtype)
        node.emit()
        return node.fx_node
    ...
    def handle_write(self, op, register, memory, elements_per_thread=None):
        node = WriteNode(self.region_graph, op, register, memory, elements_per_thread)
        node.emit()
        return node.fx_node

    def handle_construct_register_from_metadata(self, op, shape, dtype, value):
        node = ConstructRegisterFromMetadataNode(
            self.region_graph, op, shape, dtype, value
        )
        node.emit()
        return node.fx_node
    ...
    def handle_mma(self, op, lhs, rhs, acc):
        node = MmaNode(self.region_graph, op, lhs, rhs, acc)
        node.emit()
        return node.fx_node
